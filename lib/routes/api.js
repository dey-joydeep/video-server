...
import { ffprobeDurationMs } from '../ffmpeg.js';
import { listAllVideos } from '../video-utils.js';

/**
 * Creates an Express router for API endpoints.
 * @param {function(string): {status: string, hlsUrl?: string}} getHlsJobStatus - A function to get the status of an HLS job.
 * @returns {express.Router} The Express router.
 */
export default function (getHlsJobStatus) {
  ...
  router.get('/session/status', (req, res) => {
    ...
    const status = getHlsJobStatus(token);
    res.json(status);
  });
