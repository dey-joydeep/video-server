...
export async function setupHls(
  ...
  app.get('/hls/:token/:file', async (req, res) => {
    ...
    // Only synthesize a temporary playlist if master.m3u8 is NOT present yet
    if (isMaster && !fs.existsSync(p)) {
      let attempts = 0;
      ...
      // Convert to EVENT until the real playlist exists, avoid incorrect final duration
      vodSnapshot = vodSnapshot.replace(
        /#EXT-X-PLAYLIST-TYPE:VOD/,
        '#EXT-X-PLAYLIST-TYPE:EVENT'
      );
      vodSnapshot = vodSnapshot.replace(/#EXT-X-ENDLIST\n?/i, '');
    }
