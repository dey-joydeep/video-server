...
// --- PREWRITE a complete VOD playlist so players see duration/seek immediately.
async function writeFullVodMaster({
  ffprobePath,
  input,
  outDir,
  token,
  ...
  try {
    ...
    if (fs.existsSync(masterPath)) return true; // don't overwrite

    // Probe total duration using ffprobe
    const args = [
      '-v',
      'error',
      '-show_entries',
    ...
    const total = Math.max(0, parseFloat(String(out).trim()) || 0);
    if (!total || !isFinite(total)) {
      logger.warn(
        '[HLS] ffprobe total duration unavailable; skipping VOD prewrite'
      );
      return false;
    }

    const target = Math.max(1, Math.ceil(segSec + 1));
    const segCount = Math.max(1, Math.ceil(total / segSec));

    const keyPathPre = path.join(outDir, 'key.bin');
    const keyLine = fs.existsSync(keyPathPre)
    ...
    for (let i = 0; i < segCount; i++) {
      ...
      const dur = i === segCount - 1 ? remaining : segSec;
      body += `#EXTINF:${dur.toFixed(3)},\nseg_${String(i).padStart(5, '0')}.ts\n`;
    }
    body += '#EXT-X-ENDLIST\n';

    fs.writeFileSync(masterPath, body);
    logger.info(
      '[HLS] wrote FULL VOD master.m3u8 with ENDLIST (segCount=%d, total=%.3fs)',
      segCount,
      total
    );
    return true;
  } catch (e) {
    logger.warn('[HLS] VOD prewrite failed', e);
    return false;
  }
}
...
async function buildHlsForToken({
  ...
  const masterPath = path.join(outDir, 'master.m3u8');
  const writerPlaylistPath = path.join(outDir, 'writer.m3u8');
  ...
  await writeFullVodMaster({ ffprobePath, input, outDir, token, segSec });
  ...
  const argsCopy = [
    ...
    writerPlaylistPath, // The writer playlist file (e.g., writer.m3u8) or main HLS playlist file (e.g., master.m3u8)
  ];
  ...
  const baseTranscode = [
    ...
    '-hls_key_info_file', // Specify the file containing AES encryption key info
    keyInfoPath, // Path to the key info file
    writerPlaylistPath, // The main HLS playlist file (e.g., master.m3u8)
  ];
  ...
  const ready = await waitForFile(writerPlaylistPath, {
    timeoutMs: 60000,
    intervalMs: 200,
  });
...
export async function setupHls(
  ...
  app.get('/hls/:token/:file', async (req, res) => {
    ...
    if (!fs.existsSync(p)) {
      // If a segment is requested before ffmpeg has produced it, wait briefly.
      if (file.endsWith('.ts')) {
        try {
    ...
    if (file.endsWith('.m3u8')) {
      ...
      // Serve the on-disk playlist only
