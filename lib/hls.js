...
export function getHlsJobStatus(token) {
  ...
  if (hasMaster && segs >= config.MIN_READY_SEGMENTS && hasFirst) {
    ...
    if (isDebug()) resp._debug = { hasMaster: true, segCount: segs, firstSeg: hasFirst };
    return resp;
  }
 ...
 */
async function _buildHlsForToken({
  ffmpegPath,
  ffprobePath,
  input,
  outDir,
  ...
  ensureDir(outDir);
  // Build playlist progressively via ffmpeg; no prewritten VOD master
  ...
  const argsCopy = [
    ...
    '-hls_playlist_type', // VOD provides fixed duration and ENDLIST
    'vod',
...
// (removed synthetic master generator)
...
export async function setupHls(
  app,
  {
    ...
    segSec: _segSec = defaultOpts.segSec,
    tokenTtlSec = defaultOpts.tokenTtlSec,
    pinIp = defaultOpts.pinIp,
    ffmpegPath: _ffmpegPath = defaultOpts.ffmpegPath,
    ffprobePath: _ffprobePath = defaultOpts.ffprobePath,
    forceTranscode: _forceTranscode = defaultOpts.forceTranscode,
  ...
  app.get('/api/session', async (req, res) => {
    try {
      ...
      // Prefer prebuilt HLS VOD at .hls/<id>
      const prebuiltDir = path.join(absHls, id);
      let dir = prebuiltDir;
      if (!fs.existsSync(prebuiltDir)) {
        // fall back to per-token dir (will be 202)
        dir = path.join(absHls, token);
      }
      ensureDir(dir);
      const s = {
        id,
      ...
      const masterPlaylistPath = path.join(dir, 'master.m3u8');
      if (fs.existsSync(masterPlaylistPath) && firstSegReady(dir)) {
        // Register a synthetic job entry so getHlsJobStatus can verify readiness for SSE
        HLS_JOBS.set(token, { proc: null, startedAt: Date.now(), outDir: dir, id });
        JOBS_BY_VIDEO.set(id, { token, outDir: dir, startedAt: Date.now() });
        return res.json({ token, hlsUrl: `/hls/${token}/master.m3u8`, status: 'ready' });
      }

      // No prebuilt yet; do not start live transcoding here. Let tools pipeline build it.
  ...
  app.get('/hls/:token/:file', async (req, res) => {
    ...
    if (file.endsWith('.m3u8')) {
      ...
      try {
        ...
        const before = text;
        text = text.replace(/\/hlskey\/[A-Za-z0-9_-]+\/key\.bin/g, `/hlskey/${req.params.token}/key.bin`);
